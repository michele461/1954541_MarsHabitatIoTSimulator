...
1. As an operator, I want to view the real-time value of the greenhouse_temperature sensor so that I can ensure the plants do not freeze. (NFR: Fast UI update based on the in-memory cache)
2. As an operator, I want to monitor the entrance_humidity so that optimal vital parameters are maintained.
3. As a life support manager, I want to see the co2_hall levels so that the air remains breathable.
4. As a botanist, I want to check the hydroponic_ph so that nutrients are correctly absorbed by the crops.
5. As a logistics operator, I want to view the water_tank_level so that the habitat does not run out of water resources.
6. As a safety officer, I want to monitor the corridor_pressure so that fatal depressurizations are prevented.
7. As the base doctor, I want to see the PM 1, 2.5, and 10 levels from air_quality_pm25 so that I can assess the breathable air quality.
8. As the base doctor, I want to monitor the volatile organic compounds (air_quality_voc) so that intoxications are prevented.

### Telemetry Monitoring (Pub/Sub Stream)
9. As an energy engineer, I want to view the power generated by the mars/telemetry/solar_array so that I can ensure the base has sufficient energy.
...
11. As an operator, I want to view the real-time status of mars/telemetry/life_support so that critical systems are verified as operational.
12. As a maintenance worker, I want to monitor the temperature and flow of the mars/telemetry/thermal_loop so that the habitat does not overheat.
13. As an energy engineer, I want to monitor the voltage and current of the mars/telemetry/power_bus so that short circuits are prevented.
14. As an administrator, I want to see the global mars/telemetry/power_consumption so that energy rationing can be planned if necessary.
15. As an operations manager, I want to view the status of the mars/telemetry/airlock so that I know when the doors are pressurizing or depressurizing.

### Manual Actuator Control
16. As an operator, I want a dashboard button to manually toggle the cooling_fan so that I can force a temperature drop. (NFR: The UI must update the actuator state via REST POST request)
17. As an operator, I want to be able to toggle the life_support_vent so that I can manually correct oxygen levels.
18. As an operator, I want to be able to toggle the airlock_shield so that I can manually protect the base from radiation.
19. As an operator, I want to toggle the heating_unit so that the temperature does not drop below the survival threshold.
...
23. As an operator, I want the system to automatically turn ON the cooling_fan if the temperature_c > 28.0 so that the habitat does not overheat without manual intervention.
24. As a life support manager, I want the life_support_vent to turn ON automatically if the oxygen_percent < 19.5 so that personnel do not suffocate.
...
26. As an operator, I want the heating_unit to turn ON automatically if the temperature_c < 15.0 so that the plants and crew do not freeze.
